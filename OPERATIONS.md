...
| --- | --- | --- |
| New listing | Structured issue, `theme-validated`, then `approved-theme` | Publishes the freshly revalidated exact commit and deploys its tested artifact |
| Existing update | Manual `update-theme.yml` dispatch | Publishes the exact maintainer-supplied commit, archives the prior catalog snapshot, and deploys its tested artifact |
| Delisting | Manual `delist-themes.yml` dispatch | Removes complete sources, retires IDs, removes only exclusive previews, and deploys its tested artifact |
| Rename or transfer | Manual `migrate-theme-repository.yml` dispatch | Requires old/new immutable identity equality, validates canonical HEAD, and deploys its tested artifact |
...
All catalog writes share `theme-catalog-writes`. Update, delisting, and migration build and test with read-only repository permissions, verify immutable artifact semantics separately, and give write permission only to the final guarded publication job. A changed `main` base aborts publication. Every catalog writer packages its tested `site/` tree, checks that it matches the guarded publication commit, and passes that exact artifact to a least-privilege deployment job. Those jobs share `github-pages-deployments` with normal Pages releases and do not depend on workflow-token pushes triggering another workflow.
